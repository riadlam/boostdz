...
import { formatPlatformCardMeta } from '../../lib/platformCardMeta';
import { ProductPlatformCard } from './ProductPlatformCard';
import { HighlightText } from './HighlightText';
import { DualTickerDecoration, PlatformCardDecoration, SingleTickerDecoration, VerticalTicker } from './VerticalTicker';

function TweetCard({ tweet, date }) {
    return (
        <div className="w-[240px] shrink-0 rounded-2xl border border-foreground/10 bg-background p-3.5 shadow-md ring-1 ring-foreground/5">
            ...
            <p className="mt-3 line-clamp-3 text-sm leading-relaxed" dir="auto">
                {tweet.text}
            </p>
...
function TwitterDecoration() {
    ...
    const tweets = t('platforms.demoTweets', { returnObjects: true });
    const date = t('platforms.demoTweetDate');
        ...
        <PlatformCardDecoration gradient="bg-[radial-gradient(circle,#000_0%,#1da1f2_40%,transparent_100%)]">
            ...
            >
                <div className="ticker-track ticker-down flex flex-col gap-3 p-3" style={{ '--ticker-duration': '45s' }}>
                    {track.map((tweet, i) => (
                        <TweetCard key={`${tweet.handle}-${i}`} date={date} tweet={tweet} />
                    ))}
                </div>
            </div>
...
export function PlatformsSection() {
    ...
    const platformCards = t('platforms.cards', { returnObjects: true });
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                const response = await contentApi.platformCards();
                const next = {};
    ...
    const resolveMeta = useMemo(
        () => (card) => {
            const pricing = cardPricing[card.id];

            if (pricing) {
                return formatPlatformCardMeta(pricing);
            }
    ...
    return (
        <section id="platforms" className="bg-background pt-6 pb-12 sm:pt-8 sm:pb-16 md:pt-12 md:pb-20 xl:pt-16">
            <div className="mx-auto max-w-6xl px-4 md:px-6">
                <div className="flex flex-col items-center gap-3 text-center sm:gap-4">
                    <h2 className="text-2xl leading-tight font-semibold tracking-tighter text-balance sm:text-4xl lg:text-6xl dark:text-white">
                        {t('platforms.title')}
                    </h2>
                    <p className="mx-auto mt-4 max-w-2xl text-sm text-muted-foreground sm:text-sm">
                        <HighlightText
                            text={t('platforms.subtitle')}
                            highlight={t('platforms.platformsHighlight')}
                            underlineClassName="font-medium text-foreground/70"
                        />
                    </p>
                </div>

                <div className="mt-8 grid gap-3 sm:mt-12 sm:gap-2 md:mt-16 md:grid-cols-2">
                    <ProductPlatformCard
                        icon={InstagramIcon}
                        title={ig.name}
                        description={ig.body}
                        ...
                        rating={ig.rating}
                    >
                        <DualTickerDecoration platform="instagram" leftStart={1} rightStart={6} />
                    </ProductPlatformCard>
                    ...
                    >
                        <PlatformCardDecoration gradient="bg-[radial-gradient(circle,#00f2ea_0%,#ff0050_45%,transparent_100%)]">
                            <div className="grid h-full grid-cols-2 overflow-hidden rounded-xl border border-foreground/10 opacity-65 shadow-xl ring-1 ring-foreground/5 dark:opacity-50">
                                <VerticalTicker direction="bottom" platform="tiktok" startIndex={1} speed={4} />
                                <VerticalTicker direction="top" platform="tiktok" startIndex={6} speed={3} />
                            </div>
                        </PlatformCardDecoration>
                    </ProductPlatformCard>
